                         ...
                         uint32_t offset, uint8_t *buffer, const uint32_t size)
{
  switch ( ctx_type ) {
    ...
    case CONTEXT_FUOTA:
#if defined (USE_FUOTA)
      app_log_info("FUOTA restoring at offset: %" PRIu32 "\n", offset);
                       ...
                       uint32_t offset, const uint8_t *buffer, const uint32_t size)
{
  switch ( ctx_type ) {
    ...
    case CONTEXT_FUOTA:
#if defined (USE_FUOTA)
      app_log_info("FUOTA storing at offset: %" PRIu32 "\n", offset);
                                   ...
                                   uint32_t offset, uint8_t nb_page)
{
  ...
  app_log_debug("Erasing Store and Forward: %u pages from offset %" PRIu32 "\n", nb_page, offset);
