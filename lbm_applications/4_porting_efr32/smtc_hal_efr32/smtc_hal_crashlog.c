...
static volatile bool crashlog_status_normalizer = 0x1; // solve new behaviour introduce with gcc11 compiler
bool hal_crashlog_get_status()
{
  return crashlog_available_noinit & crashlog_status_normalizer; // NOSONAR: intentional bitwise AND with volatile normalizer (gcc11 .noinit workaround)
}
