...
void hal_bootloader_read_storage(uint32_t slotId, uint32_t offset, uint8_t* buffer, uint32_t size)
{
  ...
  if (err != BOOTLOADER_OK) {
    app_log_error("Error reading from flash at offset %" PRIu32 " with error: %" PRId32 "\n", offset, err);
  }
}
...
int32_t hal_bootloader_get_slot_info()
{
  ...
  app_log_info("Bootloader version: %" PRIu32 ".%" PRIu32 "\r\n",
               (uint32_t)((bootloader_info.version & 0xFF000000) >> 24),
               (uint32_t)((bootloader_info.version & 0x00FF0000) >> 16));

  err = bootloader_getStorageSlotInfo(BOOTLOADER_STORAGE_SLOT, &slot_info);

  if (err == BOOTLOADER_OK) {
    app_log_info("Slot start @ 0x%08" PRIx32 ", size %" PRIu32 " bytes\r\n", slot_info.address, slot_info.length);
  } else {
    app_log_error("Failed to get storage slot info: %" PRId32 "\r\n", err);
  }
...
bool hal_bootloader_verify_application(void)
{
  ...
  if (err != BOOTLOADER_OK) {
    app_log_error("Application verification failed: %" PRId32 " \r\n", err);
  } else {
...
void hal_bootloader_erase_slot_if_needed(void)
{
  ...
  if (err != BOOTLOADER_OK) {
    app_log_error("Failed to read storage: %" PRId32 "\r\n", err);
  } else if (dirty) {
...
void hal_bootloader_read_modify_write(uint32_t slotId, uint32_t offset, const uint8_t* buffer, uint32_t size)
{
  ...
  int32_t err = bootloader_writeStorage(slotId, offset, (uint8_t *)buffer, size);
  if (err == BOOTLOADER_OK) {
    return;
  }

  app_log_warning("Direct write failed, attempting read-modify-write at offset %" PRIu32 " size %" PRIu32 ", error: %" PRId32 "\n",
                  offset, size, err);
  ...
  if (bootloader_getStorageSlotInfo(slotId, &slotInfo) != BOOTLOADER_OK) {
    app_log_error("Failed to get storage slot info for slot %" PRIu32 "\n", slotId);
    return;
  }
  ...
  do {
    ...
    if (err != BOOTLOADER_OK) {
      app_log_error("Failed to read storage page at offset %" PRIu32 ", error: %" PRId32 "\n", page_start_offset, err);
      return;
    }
    ...
    if (err != BOOTLOADER_OK) {
      app_log_error("Failed to write storage page at offset %" PRIu32 ", error: %" PRId32 "\n", page_start_offset, err);
      return;
    }
  ...
  app_log_info("FUOTA read-modify-write completed successfully at offset %" PRIu32 "\n", offset);
}
