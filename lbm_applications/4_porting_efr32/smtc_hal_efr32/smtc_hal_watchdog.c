...
void hal_watchdog_reload(void)
{
  ...
  // Do NOT block on sl_hal_wdog_wait_sync() here: WDOG0 is clocked by ULFRCO and,
  // with the default init (debug_run/em1/em2/em3_run = false), its clock domain
  // only advances in EM0. In low-power flows (e.g. multicast Class C / FUOTA) the
  // MCU sleeps in EM2, so SYNCBUSY can stay set and the sync wait would spin
  // forever, hanging the device. Feeding is asynchronous and sl_hal_wdog_feed()
  // already skips the CLEAR command while a previous one is still synchronizing,
  // so a plain feed is safe and sufficient.
  // sl_hal_wdog_wait_sync(WDOG0);
}
