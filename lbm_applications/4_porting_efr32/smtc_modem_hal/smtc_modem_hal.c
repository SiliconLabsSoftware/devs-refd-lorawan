...
void smtc_modem_hal_irq_config_radio_irq(void (*callback)(void *context),
                                         void *context)
{
  radio_dio_irq.pin = RADIO_DIO1_PIN;
  radio_dio_irq.port = RADIO_DIO1_PORT;
  radio_dio_irq.pull_mode = RADIO_DIO1_PULL_MODE;
  radio_dio_irq.irq_mode = RADIO_DIO1_IRQ_MODE;
...
#if LBM_SP3T_RF_SWITCH_MANUAL
void smtc_modem_hal_set_ant_switch_wmbus(bool is_enable)
{
  // Only one throw of the sp3t may be selected: the LoRa rx throw is released
  // before the sub-GHz one is asserted
  hal_gpio_set_value(RADIO_SP3T_RF_SWITCH_VC2_PORT, RADIO_SP3T_RF_SWITCH_VC2_PIN, 0);
  hal_gpio_set_value(RADIO_SP3T_RF_SWITCH_VC3_PORT, RADIO_SP3T_RF_SWITCH_VC3_PIN, is_enable);
}
#endif
...
void smtc_modem_hal_set_ant_switch(bool is_tx_on)
{
  ...
  #elif LBM_SP3T_RF_SWITCH_MANUAL
  /*
   *  RF switch ctrl pins    RF switches         Action
   *  VC1   VC2   VC3        RF1  RF2  RF3
   *  ---   ---   ---        ---  ---  ---
   *  0     1     0               x           LoRa RX
   *  1     0     0          x                LoRa TX
   *  0     0     1                      x    FG28 RX/TX
   *  0     0     0                            Off
   */
  // VC1 is DO2, no need to set it
  hal_gpio_set_value(RADIO_SP3T_RF_SWITCH_VC3_PORT, RADIO_SP3T_RF_SWITCH_VC3_PIN, 0);
  hal_gpio_set_value(RADIO_SP3T_RF_SWITCH_VC2_PORT, RADIO_SP3T_RF_SWITCH_VC2_PIN, !is_tx_on);
  #else
  (void)is_tx_on;
  #endif
}
