...
if [ "$GITHUB_EVENT_NAME" == "pull_request" ]; then
    ...
    CURRENT_ANALYSIS_BRANCH="${PR_SOURCE}"
    ARGS+=(
    ...
    CURRENT_ANALYSIS_BRANCH="${BRANCH_NAME}"
...
# Reference branch cannot equal the branch being analyzed (SonarQube rejects that).
# Skip on main/default analysis; keep it for feature/release/develop branch scans.
if [ -n "$SONAR_ANALYSIS_REF_BRANCH" ]; then
    if [ "$CURRENT_ANALYSIS_BRANCH" = "$SONAR_ANALYSIS_REF_BRANCH" ]; then
        echo ">>> [sonar_scanner.sh] Skipping sonar.newCode.referenceBranch='${SONAR_ANALYSIS_REF_BRANCH}' (matches current branch)."
    else
